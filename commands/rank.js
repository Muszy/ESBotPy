...
exports.run = (bot, msg, args) => {
    ...
    if (args.length > 1) {
        ...
        if (args[0].toLowerCase() == "global" && args[1].toLowerCase() == "rep") {
            ...
            for (var num in rep) {
                if (rep[num][0] == msg.author.id) {
                    userPlace = parseInt(num) + 1;
                    userRep = rep[num][1];
                }
            }
            genList(bot, msg, "Global", "Rep", rep.splice(0, 10), userPlace, userRep);
            return;
        }
    }

    let cash = [];
    let userPlace, userCash;
    for (var id in userSettings) {
    ...
    for (var num in cash) {
        if (cash[num][0] == msg.author.id) {
            userPlace = parseInt(num) + 1;
            userCash = cash[num][1];
        }
    }
    //console.log(cash.splice(0, 10));
    genList(bot, msg, "Global", "Cash", cash.splice(0, 10), userPlace, userCash);
}
...
function guildSort(bot, msg, type, cred) {
    let users = [];
    let place, dia;
    ...
    for (var id in users) {
        if (users[id][0] == msg.author.id) {
            place = parseInt(id) + 1;
            dia = users[id][1];
        }
    }
    //console.log(users.splice(0, users.length));
    genList(bot, msg, type, cred, users.splice(0, users.length), place, dia);
}

function guildSortRep(bot, msg, type, cred) {
    let users = [];
    let place, rep;
    for (var help in userSettings) {
        if (msg.guild.member(help) != null) {
    ...
    //console.log(users.splice(0, users.length));
    genList(bot, msg, type, cred, users.splice(0, users.length), place, rep);
}

function genList(bot, msg, type, cred, list, userPlace, userCred) {
    ...
    bot.fetchUser(list[0][0]).then(function(user) {
        ...
        if (list.length == 1) {
            pushList(bot, msg, type, cred, names, list, userPlace, userCred);
            return;
        }
...
function pushList(bot, msg, type, cred, names, list, userPlace, userCred) {
    //console.log(names);
    //console.log(list);
    let toSend = [];
    ...
    for (var i = 0; i < names.length; i++) {
        ...
        if (i == names.length - 1) {
            toSend.push("-------------------------------------");
            toSend.push("Your Placing:");
            toSend.push("[" + userPlace + "] " + msg.author.username + " | " + cred + ": " + userCred);
            toSend.push("```");
