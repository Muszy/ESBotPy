...
bot.login(config.token, function(err, token) {
	...
	if (!token) { console.log("***failed to connect***"); setTimeout(() => { process.exit(1); }, 2000); }
});
...
function eventChecker() {
	...
	if(globals["event"].end != "" && event==null && count > 457230000) {
		...
		event = schedule.scheduleJob(date, function(){
		    ...
		    for(var id in serverSettings){
				...
				//bot.channels.get(ch).sendMessage('tesla');
...
//================chat listeners-==============================
		...
		if ( msg.content.toLowerCase().indexOf("when") > -1 && (msg.content.toLowerCase().indexOf("gacha") > -1 || msg.content.toLowerCase().indexOf("scout") > -1) && msg.content.toLowerCase().indexOf("announce") > -1) {
		...
		if ( msg.content.toLowerCase().indexOf("when") > -1 && msg.content.toLowerCase().indexOf("event") > -1 && msg.content.toLowerCase().indexOf("announce") > -1) {
