...
exports.run = (bot, msg, args) => {
	...
	file[msg.guild.id].gNotifyChannel = msg.channel.id;

	fs.writeFile(fileName, JSON.stringify((file)), function (err) {
 		if (err) return console.log(err);
		//console.log(JSON.stringify(file));
		console.log('updating gbf notification channel in ' + fileName);
	});

	updateSettings();

	let embed = new discord.RichEmbed();
	let channel = msg.guild.channels.get(file[msg.guild.id].gNotifyChannel);

	embed.setTitle("Confirmation:")
		.setColor(0x3399FF)
		.setThumbnail("http://i.imgur.com/7TL0t99.png")
		.setDescription("GBF Notification channel is now: **" + channel.name + "**.");

	msg.channel.sendEmbed(embed).catch(console.error);
}

exports.help = (bot, msg, args) => {
	return "Use `gnotifyhere` to change the GBF notification channel.";
}
