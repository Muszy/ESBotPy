...
var scout = require("./lim.js");
var eventPull = require("./lib/limPull.js");
...
var download = function(msg, uri, filename, callback) {
    request.head(uri, function(err, res, body) {
        if (err) {
            ...
            msg.channel.send({errorMsg}).then(m => m.delete(4000)).catch(console.error);
            msg.delete(1500);
            return;
        }
...
exports.run = (bot, msg, args) => {
    ...
    if (args.length == 0) {
        ...
        let embed = new discord.RichEmbed();
                embed.setTitle("List of Limited Gacha Available:")
                    .setColor(0x96F08C)
                    .setDescription("lim1 : 1 Day Wedding\nlim2 : Summer Sky Ocean Blue\nlim3 : One Night Magic\nlim4 : Bright Christmas\nlim5 : Sunrise of Youth")
            ...
            gm(dir + 'bbase' + msg.author.id + '.png')
                .write(dir + 'bbase' + msg.author.id + '.png', function(err) {
                    if (!err) {
                        ...
                        console.log(err);
                        msg.author.send({scoutError}).catch(console.error);
                        msg.author.send({file: dir + '10pull.png'});
                        return;
                    }
                });
        });
        ...
        gm(dir + 'bbase' + msg.author.id + '.png').append(dir + 'brow' + msg.author.id + '.png')
            .write(dir + 'bscout' + msg.author.id + '.png', function(err) {
                if (!err) {
                    ...
                    msg.author.send({file: dir + 'bscout' + msg.author.id + '.png'}).then(
                        function() {
                    ...
                    note.setTitle("Finished!")
                        ...
                        .setThumbnail("http://i.imgur.com/7TL0t99.png");
                    msg.channel.send({note}).catch(console.error)*/
